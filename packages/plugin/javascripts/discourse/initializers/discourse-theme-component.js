...
const matchProductionHost = () => {
  return window.location.host === 'forum.makerdao.com';
};
...
const queryBadgesAPI = (data) => {
  ...
  const properURL = matchProductionHost()
    ? ThemeConfig.production.lambdaUrl(data)
    : ThemeConfig.development.lambdaUrl(data);
      ...
      try {
        ...
        errorsEmpty
          ? (document.getElementById(
              'badge-status'
            ).innerText = `${count} new Maker badge${count > 1 ? 's' : ''} ha${
              count > 1 ? 've' : 's'
            } been awarded to your profile!`)
          : (document.getElementById('badge-status').innerText =
              'No new eligible badges found.');
      } catch (error) {
        document.getElementById('badge-error').innerText =
          'Badges API - JSON Parse Error';
